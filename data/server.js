...
function deviceFingerprint(req) {
  try {
    ...
    const bn = (r.browser && r.browser.name) || 'unknown';
    const bv = (r.browser && r.browser.major) || '0';
    const on = (r.os && r.os.name) || 'unknown';
    const ov = (r.os && r.os.version) || '0';
    const dm = (r.device && r.device.model) || 'desktop';
    const fp = `${bn}|${bv}|${on}|${ov}|${dm}`;
    return crypto.createHash('sha256').update(fp).digest('hex').slice(0, 32);
  } catch (e) {
    return crypto.createHash('sha256').update(req.headers['user-agent'] || 'unknown').digest('hex').slice(0, 32);
  }
}
...
function userAgentSummary(req) {
  try {
    ...
    const parser = new UAParser(ua);
    const r = parser.getResult();
    const bn = (r.browser && r.browser.name) || '?';
    const bv = (r.browser && r.browser.major) || '';
    const on = (r.os && r.os.name) || '?';
    const ov = (r.os && r.os.version) || '';
    return `${bn} ${bv} / ${on} ${ov}`.trim();
  } catch (e) {
    return 'unknown';
  }
}
...
app.post('/api/login', (req, res) => {
  ...
  res.json({ ok: true, user: { id: user.id, name: user.name, email: user.email } });
    ...
    res.status(500).json({ error: 'Greška pri prijavi. Pokušajte ponovo.' });
  }
});
